    ...
    The plotted results are shown first for clarity. Figure~\ref{fig:comparison} summarizes the aggregate metric comparison between the two retrieval settings, while Figure~\ref{fig:heatmap-rerank} shows the per-question behavior for the reranked pipeline. These visualizations make it easier to see not only the overall trend, but also where performance varies question by question.
...
\includegraphics[width=\linewidth]{comparison_chart.png}
    ...
    \begin{figure}[h]
    \centering
\includegraphics[width=\linewidth]{heatmap_with_rerank.png}
    \caption{Per-question metric heatmap for the \texttt{with\_rerank} configuration.}
    \label{fig:heatmap-rerank}
    \end{figure}

    \subsection{Quantitative Results}
    Table~\ref{tab:eval-agg} reports the aggregate means across all 15 evaluation questions. The comparison shows that the \texttt{without\_rerank} configuration achieved slightly higher Answer Relevancy and noticeably higher Faithfulness and Contextual Precision on this benchmark slice. In particular, the strongest improvement appears in Contextual Precision, suggesting that the baseline retrieval setup preserved useful supporting evidence more consistently for the tested queries.
